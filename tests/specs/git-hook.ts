...
export default testSuite(({ describe }) => {
	describe('Git hook', ({ test }) => {
		...
		test('errors when not in Git repo', async () => {
			...
			const { exitCode, stderr } = await lazycommit(['hook', 'install'], {
				reject: false,
			});
		...
		test('installs from Git repo subdirectory', async () => {
			...
			const { stdout } = await lazycommit(['hook', 'install'], {
		...
		test('Commits', async () => {
			...
			expect(typeof commitMessage === 'string' ? commitMessage.startsWith('# ') : false).not.toBe(true);
